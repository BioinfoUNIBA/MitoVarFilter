...
fasta_dir='/lustrehome/sharonnatashacox/NewMToolBox/MToolBox/MToolBox/data/new_chrM_ss/'
#mtdna_fasta='chrRSRS.fa'
mtdna_fasta='chrM.fa'
inputfile=''
#hgenome_fasta='hg19RSRS.fa'
hgenome_fasta='hg38.fa'
mqual=25
clev=0.80
cov=5
glen=10
...
#sexe='samtools'
sexe='/lustrehome/sharonnatashacox/.conda/envs/venv2/bin/samtools'
sversion=0
crf=0
crc=0
cru=0
...
if ext=='bam':
    ...
    #cmd1='%s sort %s.bam > %s-sorted.bam' %(sexe,basext,basext)
	#else:
	cmd1='%s sort %s.bam -o %s-sorted.bam' %(sexe,basext,basext)
	cmd2='%s index %s-sorted.bam' %(sexe,basext)
	cmd3='%s view -o %s.sam %s.bam' %(sexe,basext,basext)
	os.system(cmd1)
	os.system(cmd2)
	os.system(cmd3)
